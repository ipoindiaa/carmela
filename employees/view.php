...
$id = get('id');
$emp = $db->fetch("SELECT e.*, a.current_balance as advance_balance FROM employees e LEFT JOIN accounts a ON a.id = e.advance_account_id WHERE e.id = ? AND e.business_id = ?", [$id, $businessId]);
...
<div class="stats-grid" style="grid-template-columns: repeat(3,1fr);">
    ...
    <div class="stat-card"><div class="stat-value text-yellow"><?= formatAmount($emp['advance_balance'] ?? 0) ?></div><div class="stat-label">Advance Outstanding</div></div>
