...
CREATE TABLE `journal_entries` (
    ...
    `transaction_type` ENUM('CAR_PURCHASE','PURCHASE_PAYMENT_REPAIR','PURCHASE_AMOUNT_CORRECTION','CAR_TOKEN_RECEIVED','CAR_SALE','RTO_EXPENSE','RTO_RECOVERY','CAR_EXPENSE','GENERAL_EXPENSE','JOURNAL_VOUCHER','PARTNER_INVEST','PARTNER_WITHDRAW','PARTNER_SETTLEMENT','SALARY_PAYMENT','EMPLOYEE_COMMISSION','EMPLOYEE_ADVANCE','EMPLOYEE_ADVANCE_WRITEOFF','LOAN_GIVEN','LOAN_RECEIVED','LOAN_TAKEN','LOAN_REPAID','CONTRA_TRANSFER','OPENING_BALANCE','REVERSAL','BAD_DEBT','PROFIT_DISTRIBUTION','TOKEN_FORFEITURE','TOKEN_REFUND','CASH_RECONCILIATION','DEALER_COMMISSION','DEALER_COMMISSION_PAYMENT') NOT NULL,
...
CREATE TABLE `cars` (
    ...
    `purchase_amount_mode` VARCHAR(20) NOT NULL DEFAULT 'FIXED',
    ...
    `purchase_dealer_party_id` CHAR(36) DEFAULT NULL,
    `partner_id` CHAR(36) DEFAULT NULL,
...
CREATE TABLE `debtors_creditors` (
    ...
    `business_id` CHAR(36) NOT NULL,
    `name` VARCHAR(200) NOT NULL,
    `type` ENUM('DEBTOR','CREDITOR','BUYER','SELLER','DEALER') NOT NULL,
    `phone` VARCHAR(20) DEFAULT NULL,
    `email` VARCHAR(100) DEFAULT NULL,
    `address` TEXT DEFAULT NULL,
