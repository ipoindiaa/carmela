...
<div class="card">
    <div class="card-body summary-strip"><span class="text-muted">Balance As On <?= formatDate($dateTo) ?>:</span> <strong class="amount <?= ($bankBalanceAsOn['type'] ?? 'DR') === 'DR' ? 'debit-amount' : 'credit-amount' ?>"><?= formatAmount($bankBalanceAsOn['amount'] ?? 0) ?> <?= $bankBalanceAsOn['type'] ?? 'DR' ?></strong></div>
</div>

<div class="table-container table-container-fill">
    <table class="table-total-room">
        <thead><tr><th>Date / Time</th><th>Ref</th><th>Type</th><th>Car / Party</th><th>Narration</th><th class="text-right debit-amount">Deposit (Dr)</th><th class="text-right credit-amount">Withdrawal (Cr)</th><th class="text-right">Balance</th></tr></thead>
        <tbody>
