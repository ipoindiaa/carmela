...
<div class="filter-bar"><form method="get"><div><label class="form-label">Search queue</label><input type="search" name="q" class="form-control" value="<?= clean($query) ?>" placeholder="Car or financier"></div><div><label class="form-label">Priority</label><select name="priority" class="form-control"><option value="">All priorities</option><?php foreach(['HIGH','MEDIUM'] as $value): ?><option value="<?= $value ?>" <?= $priority===$value?'selected':'' ?>><?= clean(ucfirst(strtolower($value))) ?></option><?php endforeach; ?></select></div><button type="submit" class="btn btn-outline btn-sm"><i class="ri-filter-line"></i> Apply</button></form></div>
...
<?php if(!$items): ?><tr><td colspan="7" class="empty-table-cell"><div class="table-filter-empty-state"><i class="ri-checkbox-circle-line"></i><strong>No matching pending actions</strong></div></td></tr><?php endif; ?>
