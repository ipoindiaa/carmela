...
<div class="filter-bar entry-type-filter-bar">
    <form method="GET" class="entry-type-filter-form">
        ...
        <div class="form-group"><label class="form-label">Category</label><select name="category" class="form-control"><option value="">All categories</option><?php foreach ($categories as $category): ?><option value="<?= clean($category) ?>" <?= $filterCategory === $category ? 'selected' : '' ?>><?= clean($category) ?></option><?php endforeach; ?></select></div>
