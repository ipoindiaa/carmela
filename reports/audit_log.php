...
function auditLogUrl($page, $logDate = '', $logAction = '', $logEntity = '', $logModule = '') {
    $query = ['page' => $page];
...
<div class="filter-bar">
    <form method="GET">
        ...
        <?php if ($logDate !== '' || $logAction !== '' || $logEntity !== '' || $logModule !== ''): ?><a href="audit_log.php" class="btn btn-ghost btn-sm">Clear</a><?php endif; ?>
    </form>
</div>
...
<div class="pagination">
    <?php for ($i = 1; $i <= $pagination['total_pages']; $i++): ?>
        <a href="<?= clean(auditLogUrl($i, $logDate, $logAction, $logEntity, $logModule)) ?>" class="<?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
    <?php endfor; ?>
</div>
<?php endif; ?>
